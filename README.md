...
inputs/github_packages.csv        ← curated packages to sweep for GitHub repos
                                    (package, import_name, search_mode)
inputs/github_owner_blacklist.csv ← owners excluded from GitHub adoption counts
                                    (owner, reason — conda-forge, self, …)
...
data/github_repos/<package>.csv ← script-generated repo candidates per package
                                    (status = auto / exclude / manual, evidence in notes)
...
# A GitHub repo (OWNER/REPO or a github.com URL), added by hand as status=manual;
# --group is the package it imports (default: openff-toolkit). Everything else in
# data/github_repos/ is script-generated by `discover-github-repos` (see below).
...
`scholar_cluster_id` / `chemrxiv_id` may be blank. Group names are
load-bearing: e.g. papers in `publications/force-field.csv` are what the
force-field citation subtotal counts.

GitHub repo lists aren't hand-curated the same way — see
[GitHub repo imports and stars](#github-repo-imports-and-stars-datagithub_repo_starscsv)
below for `inputs/github_packages.csv`, `inputs/github_owner_blacklist.csv`,
and the script-generated `data/github_repos/<package>.csv` files.
...
These help you *find* candidate sources; most write to `candidates/` (which is
...
GitHub repo discovery is different: it verifies every candidate itself and
writes straight to `data/github_repos/<package>.csv` — there's no
`candidates/` review step for this one.

```bash
openff-stats discover-github-repos                   # sweep every package in inputs/github_packages.csv
openff-stats discover-github-repos --package descent # one package (import name + search mode read from the CSV)
```

See [GitHub repo imports and stars](#github-repo-imports-and-stars-datagithub_repo_starscsv)
below for how candidates are verified and tagged.
...
Unlike every other source, this one isn't hand-curated. `data/github_repos/<package>.csv`
(one file per package in `inputs/github_packages.csv`) is fully
script-generated by `discover-github-repos`, which searches, verifies, and
tags every candidate with no human review step.

**Discovery.** For each package, GitHub code search runs a set of queries —
declared dependencies (the conda-forge name in `setup.py`, `setup.cfg`,
`pyproject.toml`, `environment.yml`, `pixi.toml`) plus runtime imports
(`import`/`from <import name>` in `.py` and `.ipynb` files) — sharded by file
size to work around GitHub's 1,000-result-per-query cap. `requirements.txt`
is only searched with `--include-requirements`: GitHub tokenizes on
punctuation, so a short package name matches every hyphenated lookalike
(`gradient-descent` matches a `descent` query), and the query mostly adds
shard-paging time for little precision.

**Verification.** Every candidate repo is checked against the files that
actually matched it, using the search API's text-match fragments: a
dependency-declaration regex for manifest files, or a real `import X`/
`from X` statement for code. A file is only fetched when the fragment is
inconclusive. Notebook (`.ipynb`) matches inside base64-encoded cell outputs
can never satisfy the import check, so those never verify. Repos that verify
are tagged `status=auto`; repos that match the search but fail verification
are tagged `status=exclude` and kept in the CSV for the record, with the
reason in `notes`. Repos added by hand (`add-github-repo`) are
`status=manual`. Only `auto`/`manual` rows are counted — `exclude` rows are
skipped by collection.

**Search modes.** `inputs/github_packages.csv` (`package, import_name,
search_mode`) controls what gets searched per package. `search_mode=full`
(the default) runs every manifest and import query above.
`search_mode=environment-only` restricts the search to conda environment
files (`environment.yml` / `environment.yaml`) only — used for `descent`,
whose generic name drowns every other query in same-token noise (its import
query alone reports 800k+ hits). Its counts are therefore a floor: known
external users of `descent` also import `openff-toolkit`, so they're counted
there instead.
...
- `self` — owner is the OpenFF org or a core maintainer account
  (`openforcefield`, `lilyminium`, `jaclark5`, `mattwthompson`, `j-wags`,
  `Yoshanuikabundi`, `ntBre` — from `inputs/github_owner_blacklist.csv`, a
  plain `owner,reason` CSV): our own repos measure our own activity, not
  external adoption.
- `meta` — owner is `conda-forge` (also from the blacklist): packaging
  infrastructure, not usage.
- `duplicate` — the repo is listed under more than one package's group; it
  counts once, in the first group by `inputs/github_packages.csv` order, so
  later rows read as *additional* adoption rather than double-counting.
...
applies it automatically on every run.
...
stamp back to `main`. Discovery is not run in CI: `discover-github-repos` is
a manual sweep step, and the other `discover-*` commands write `candidates/`
for human review. `github-descriptions` also isn't run in CI (it needs a
hand-reviewed category column).
