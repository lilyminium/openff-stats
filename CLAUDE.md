...
- `data/github_repos/*.csv` → `github.py` → `data/github_repo_stars.csv` (+ descriptions)
  ...
  - `github-stars` prints per-group repo-import counts and star sums; `classify-repos` re-applies `external`/`reason` tagging from `inputs/github_owner_blacklist.csv` offline
